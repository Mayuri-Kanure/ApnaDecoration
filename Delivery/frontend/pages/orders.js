...
export default function OrdersPage() {
  ...
  const loadOrdersData = async () => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      const response = await axios.get(`${process.env.NEXT_PUBLIC_ORDERS_API_URL}/my-orders`, {
  ...
  const loadOrdersStats = async () => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      const response = await axios.get(`${process.env.NEXT_PUBLIC_ORDERS_API_URL}/stats`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAcceptOrder = async (orderId) => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      await axios.post(`${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/orders/${orderId}/accept`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleRejectOrder = async (orderId) => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      await axios.post(`${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/orders/${orderId}/reject`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleStartDelivery = async (orderId) => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      await axios.post(`${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/orders/${orderId}/start`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setSnackbar({ open: true, message: 'Delivery started!', severity: 'success' });
    } catch (error) {
      console.error('Error starting delivery:', error);
      setSnackbar({ open: true, message: 'Error starting delivery', severity: 'error' });
    }
  };
  ...
  const handleCompleteDelivery = async (orderId) => {
    try {
      const token = localStorage.getItem('deliveryBoyToken');
      await axios.post(`${process.env.NEXT_PUBLIC_DELIVERY_API_URL}/orders/${orderId}/complete`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setSnackbar({ open: true, message: 'Error completing delivery', severity: 'error' });
    }
  };

  const handleContactCustomer = (order) => {
    // TODO: Implement customer contact
    setSnackbar({ open: true, message: 'Contacting customer...', severity: 'info' });
  };
  ...
  return (
    <Box sx={{ 
      ...
      {/* Stats Cards */}
      <Grid container spacing={3} sx={{ mb: 4, pr: 3, mt: 2 }}>
        <Grid item xs={12} sm={6} md={2.4}>
          <Card sx={{ 
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: '#1e293b', mb: 1 }}>Total Orders</Typography>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#2F66FF', mb: 2 }}>{ordersStats.totalOrders}</Typography>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Card sx={{ 
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: '#1e293b', mb: 1 }}>Available</Typography>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#28C76F', mb: 2 }}>{ordersStats.availableOrders}</Typography>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Card sx={{ 
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: '#1e293b', mb: 1 }}>In Progress</Typography>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#FF9F43', mb: 2 }}>{ordersStats.inProgressOrders}</Typography>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Card sx={{ 
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: '#1e293b', mb: 1 }}>Completed</Typography>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#28C76F', mb: 2 }}>{ordersStats.completedOrders}</Typography>
      ...
      {/* Search and Filter */}
      <Box sx={{ mb: 4, pr: 3, mt: 2 }}>
        <Card sx={{ p: 3, borderRadius: 3, boxShadow: 3, bgcolor: 'white', border: '2px solid #e2e8f0' }}>
      ...
      <Box sx={{ pr: 3, mt: 2 }}>
        <Card sx={{ borderRadius: 3, boxShadow: 3, bgcolor: 'white', border: '2px solid #e2e8f0' }}>
          ...
          <TabPanel value={tabValue} index={0}>
            ...
            <Card sx={{ borderRadius: 3, boxShadow: 3, bgcolor: 'white', border: '2px solid #e2e8f0' }}>
              <CardContent>
                <Typography variant="h6" sx={{ mb: 2 }}>All Orders</Typography>
                <TableContainer>
                  <Table>
                    <TableHead>
                      <TableRow>
          ...
          <TabPanel value={tabValue} index={1}>
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              Available orders will be displayed here.
            </Alert>
          </TabPanel>
          ...
          <TabPanel value={tabValue} index={2}>
            ...
            <Alert severity="info" sx={{ mb: 2 }}>
              In-progress orders will be displayed here.
            </Alert>
          </TabPanel>
      ...
      {loading && (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', mt: 4 }}>
          <LinearProgress />
          <Typography variant="h6" sx={{ mt: 2 }}>Loading orders data...</Typography>
        </Box>
      )}
          ...
          {selectedOrder && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1">{selectedOrder.pickupAddress}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1">₹{selectedOrder.amount}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2">Distance</Typography>
                <Typography variant="body1">{selectedOrder.distance}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2">Estimated Time</Typography>
                <Typography variant="body1">{selectedOrder.estimatedTime}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1">{selectedOrder.priority}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2">Items</Typography>
                <Typography variant="body1">{selectedOrder.items.join(', ')}</Typography>
              </Grid>
            </Grid>
          )}
        </DialogContent>
      ...
      <Dialog open={mapDialogOpen} onClose={() => setMapDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body1">
            Map view would show delivery location and route. 
            This would integrate with Google Maps API for real-time tracking.
          </Typography>
        </DialogContent>
        <DialogActions>
