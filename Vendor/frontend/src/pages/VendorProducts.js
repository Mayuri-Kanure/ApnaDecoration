  ...
  useEffect(() => {
    ...
    if (updatedProductId && updatedProductData) {
      try {
        ...
        console.log('🔄 Found updated product in sessionStorage:', updatedProduct);
        
        // Update the specific product in the local state after a short delay
        setTimeout(() => {
          setProducts(prev => {
            console.log('🔄 Updating products array with sessionStorage data, current length:', prev.length);
            const newProducts = prev.map(product => {
              if (product._id === updatedProduct._id || product.id === updatedProduct._id) {
                console.log('🔄 Found matching product to update from sessionStorage:', product.name);
                ...
                const transformedProduct = {
                  ...
                  category: updatedProduct.category?.name || updatedProduct.category,
                ...
                console.log('🔄 Transformed updated product from sessionStorage:', transformedProduct);
                return transformedProduct;
              }
              return product;
            });
            
            console.log('✅ Products updated in local state with sessionStorage data');
            return newProducts;
          });
    ...
    const handleProductUpdate = (event) => {
      ...
      if (event.detail.updatedProduct) {
        ...
        console.log('🔄 Using updated product data from event:', updatedProduct);
        
        // Update the specific product in the local state
        setProducts(prev => {
          console.log('🔄 Updating products array from event, current length:', prev.length);
          const newProducts = prev.map(product => {
            if (product._id === updatedProduct._id || product.id === updatedProduct._id) {
              console.log('🔄 Found matching product to update from event:', product.name);
              // Transform the updated product to match frontend format
              let imageUrl = null;
              ...
              console.log('🔄 Transformed updated product from event:', transformedProduct);
              return transformedProduct;
            }
            return product;
          });
    ...
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        ...
        const transformedProducts = response.products.map(p => ({
          ...
          sku: p.sku,
          price: p.price,
          status: p.active_status || 'pending',
          category: p.category,
              ...
              <Table>
                ...
                <TableBody>
                  {filteredProducts.map((product) => {
                    console.log('📦 VendorProducts - Rendering product:', product.name);
                    console.log('📦 VendorProducts - Product ID:', product.id);
                    console.log('📦 VendorProducts - Product image:', product.image);
                    console.log('📦 VendorProducts - Product thumbnail:', product.thumbnail);
                    console.log('📦 VendorProducts - Product images array:', product.images);
                    console.log('📦 VendorProducts - Available fields:', Object.keys(product));
                    console.log('📦 VendorProducts - Full product data:', JSON.stringify(product, null, 2));
                    ...
                    return (
                      <TableRow 
                        ...
                        hover
                        sx={{
                          backgroundColor: recentlyUpdated === product.id ? '#e8f5e8' : 'inherit',
                          border: recentlyUpdated === product.id ? '2px solid #4caf50' : 'none'
                        }}
                      >
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Box
                              ...
                              onError={(e) => {
                                // Try thumbnail first
                                if (product.thumbnail && !e.target.dataset.triedThumbnail) {
                                ...
                                else if (product.images?.[0] && !e.target.dataset.triedArray) {
                                ...
                                // Final fallback - show "Add Images" placeholder
                                  ...
                                  e.target.src = 'data:image/svg+xml;base64,';
                                }
                              }}
                            />
                            <Box>
                              ...
                              {/* Show warning for products without images */}
                              {!product.image && !product.thumbnail && (!product.images || product.images.length === 0) && (
                        ...
                        <TableCell>
                          ...
                          <IconButton size="small" sx={{ ml: 1 }} onClick={() => handleEditProduct(product)}>
                            <EditIcon />
                          </IconButton>
                          <IconButton size="small" sx={{ ml: 1 }} onClick={() => handleDeleteProduct(product)}>
                            <DeleteIcon />
                          </IconButton>
                        </TableCell>
                      </TableRow>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {(() => {
                      if (selectedProduct.status === 'pending') return 'Waiting for admin approval';
                      if (selectedProduct.status === 'approved') return 'Visible to customers';
                      if (selectedProduct.status === 'rejected') return 'Needs corrections';
                    })()}
                  </Typography>
                </Box>
              </Grid>
