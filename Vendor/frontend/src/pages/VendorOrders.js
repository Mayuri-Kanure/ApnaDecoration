          ...
          {selectedOrder && (
            <Box>
              <Grid container spacing={2} sx={{ mb: 3 }}>
                ...
                <Grid item xs={6}>
                  <Typography variant="body2" color="#64748b">Order Date</Typography>
                  <Typography variant="body1">
              ...
              {selectedOrder.items?.map((item, index) => (
                <Box key={index} sx={{ p: 2, border: '1px solid #e2e8f0', borderRadius: 1, mb: 1 }}>
                  <Typography variant="body1" fontWeight="medium">
                    {item.product?.name || item.productName || 'Product'}
                  </Typography>
                  <Typography variant="body2" color="#64748b">
                    Quantity: {item.quantity} × {formatCurrency(item.unitPrice || item.price)}
                  </Typography>
                </Box>
              ))}
              ...
              <Box sx={{ mt: 3, p: 2, backgroundColor: '#f8fafc', borderRadius: 1 }}>
                <Typography variant="h6" sx={{ mb: 1 }}>
                  Total: {formatCurrency(selectedOrder.pricing?.total || selectedOrder.total || 0)}
                </Typography>
              </Box>
            </Box>
          )}
