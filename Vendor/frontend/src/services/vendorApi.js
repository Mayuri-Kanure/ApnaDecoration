...
class VendorApiService {
  ...
  async updateVendorProductWithFiles(
    id,
    productData,
    files,
    removedImages = [],
    ...
    try {
      ...
      console.log(
        "🔍 Making request to:",
        `${this.baseURL}/vendor-products/${id}/files`,
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "❌ Vendor API Error Response (Update Files):",
          errorText,
        );
  ...
  async getVendorNotifications() {
    return this.request("/vendor-notifications");
  }
}
