...
const FCM_STORAGE_KEY = "fcmVendorDeviceToken";
const PUSH_API = `${API_BASE_URL}/push-notifications`;
...
class PushNotificationService {
  constructor() {
    ...
    this.deviceToken =
      typeof localStorage !== "undefined"
        ? localStorage.getItem(FCM_STORAGE_KEY)
        : null;
  ...
  async attachNativeListeners(PushNotifications) {
    ...
    await PushNotifications.addListener("registration", (token) => {
      ...
      localStorage.setItem(FCM_STORAGE_KEY, value);
  ...
  async initialize(requestPermission = false) {
    try {
      ...
      if (cached && localStorage.getItem(AUTH_TOKEN_KEY)) {
  ...
  async disablePush() {
    const token =
      this.deviceToken || localStorage.getItem(FCM_STORAGE_KEY);
  ...
  async resumeAfterLogin() {
    if (!this.isNative || !localStorage.getItem(AUTH_TOKEN_KEY)) return;
