...
// phone, email, employeeId have unique: true, so no explicit indexes needed
...
deliveryBoySchema.methods.isWorking = function () {
  ...
  const currentTime = now.toTimeString("en-US", {
    hour12: false,
    hour: "2-digit",
    minute: "2-digit",
  });
...
deliveryBoySchema.methods.updatePerformanceMetrics = function (
  deliveryStatus,
  deliveryTime = null,
) {
  ...
  if (deliveryStatus === "delivered") {
    ...
    if (deliveryTime) {
      ...
      this.averageDeliveryTime =
        (currentAvg * (this.successfulDeliveries - 1) + deliveryTime) /
...
deliveryBoySchema.statics.getDeliveryBoysByLocation = function (
  latitude,
  longitude,
  radius = 5,
) {
...
deliveryBoySchema.statics.getDeliveryBoyStatistics = function (
  startDate,
  endDate,
) {
  const matchStage = {};
    ...
    { $match: matchStage },
    {
      $group: {
        ...
        availableDeliveryBoys: {
          $sum: {
            $cond: [
              { $eq: ["$status", "active"], $eq: ["$isAvailable", true] },
              1,
              0,
            ],
          },
