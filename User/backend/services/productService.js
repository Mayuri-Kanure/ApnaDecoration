...
class ProductService {
  ...
  static async getProducts(filters = {}) {
    ...
    const regularProductQuery = { status };
    ...
    const vendorProductQuery = { status: 'approved' };
    ...
    const total = await Product.countDocuments(regularProductQuery) + 
  ...
  static async createProduct(productData, userId) {
    ...
    if (!productData.sku) {
      productData.sku = `PRD-${Date.now()}-${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
    }
