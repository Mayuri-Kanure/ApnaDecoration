...
// Create banner (admin)
router.post('/', authMiddleware, authorize(['admin', 'manager']), upload.single('image'), async (req, res) => {
  try {
...
// Update banner (admin)
router.put('/:id', authMiddleware, authorize(['admin', 'manager']), upload.single('image'), async (req, res) => {
  try {
