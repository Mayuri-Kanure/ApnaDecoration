...
// Public route for getting vendor products (no auth required)
router.get("/", vendorProductController.getPublicVendorProducts);

// Public route for getting single vendor product by ID (no auth required)
router.get("/public/:id", vendorProductController.getPublicVendorProduct);

// Get all vendor products (admin only)
router.get("/admin", authMiddleware, vendorProductController.getVendorProducts);

// Get new vendor product requests (admin only)
router.get(
  "/new-requests",
  authMiddleware,
  vendorProductController.getNewRequests,
);
...
router.get(
  "/approved",
  authMiddleware,
  vendorProductController.getApprovedProducts,
);
...
router.get(
  "/denied",
  authMiddleware,
  vendorProductController.getDeniedProducts,
);
...
router.get(
  "/status/pending",
  authMiddleware,
  vendorProductController.getPendingProducts,
);
...
router.put(
  "/approve/:id",
  authMiddleware,
  vendorProductController.approveProduct,
);
...
router.get(
  "/my-products",
  authMiddleware,
  vendorProductController.getMyVendorProducts,
);
...
router.put(
  ...
  authMiddleware,
  uploadVendorImages,
  vendorProductController.updateVendorProductWithFiles,
);
