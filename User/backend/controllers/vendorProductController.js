...
exports.createVendorProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      console.log('🔍 Backend - File details:', req.files.map(f => ({ originalname: f.originalname, filename: f.filename, path: f.path })));
    }
    ...
    console.log('🔍 Backend - Extracted data:', { name, description, price, category, sku, stock, vendorId });
    ...
    const generatedSKU = sku || `VND-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    
    // Handle images
    let images = [];
    if (req.files && req.files.length > 0) {
      // multer with upload.array('images') provides req.files directly as an array
      images = req.files.map(file => `/uploads/vendor-products/${file.filename}`);
...
exports.getVendorProduct = async (req, res) => {
  try {
    ...
    const vendorProduct = await VendorProduct.findOne({ _id: id, vendorId: req.user.id });
...
exports.updateVendorProduct = async (req, res) => {
  try {
    ...
    if (vendorProduct.status === 'approved') {
      return res.status(400).json({ success: false, message: 'Cannot update approved product' });
    }
...
exports.updateVendorProductWithFiles = async (req, res) => {
  ...
  try {
    ...
    if (removedImages.length > 0) {
      filteredImages = existingImages.filter(img => !removedImages.includes(img));
    ...
    if (req.files && req.files.length > 0) {
      ...
      const newImagePaths = req.files.map(file => {
        ...
        console.log('🔍 Backend - Relative path after replacement:', relativePath);
      ...
      console.log('🔍 Backend - Final images after adding new ones:', updateData.images);
      console.log('🔍 Backend - Total images after update:', updateData.images.length);
      ...
      if (!vendorProduct.thumbnail && updateData.images.length > 0) {
        ...
        console.log('🔍 Backend - Existing thumbnail:', vendorProduct.thumbnail);
      }
    } else {
    ...
    console.log('🔍 Backend - Updated product thumbnail:', updatedProduct.thumbnail);
    console.log('🔍 Backend - Updated product full data:', JSON.stringify(updatedProduct, null, 2));
...
exports.deleteVendorProduct = async (req, res) => {
  try {
    ...
    if (vendorProduct.status === 'approved') {
      return res.status(400).json({ success: false, message: 'Cannot delete approved product' });
    }
