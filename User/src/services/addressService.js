...
const addressService = {
  ...
  validateAddress: async (addressData) => {
    try {
      const response = await apiService.post('/addresses/validate', addressData);
      return response;
    } catch (error) {
  ...
  calculateShippingCost: async (addressId, cartItems) => {
    try {
      const response = await apiService.post(`/addresses/${addressId}/shipping-cost`, {
        cartItems
      });
      return response;
    } catch (error) {
  ...
  checkServiceability: async (addressData) => {
    try {
      const response = await apiService.post('/addresses/check-serviceability', addressData);
      return response;
    } catch (error) {
