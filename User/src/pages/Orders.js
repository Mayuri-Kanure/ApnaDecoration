...
import { Package, Truck, Check, Clock, X, RefreshCw, Calendar, MapPin, Users, HelpCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Track and manage your decoration orders</p>
        </div>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {['all', 'pending', 'confirmed', 'processing', 'ready', 'completed', 'cancelled'].map((status) => (
              ...
              >
                {status === 'ready' ? 'Ready for Setup' : status.charAt(0).toUpperCase() + status.slice(1)}
              </button>
            ))}
          </div>
        </div>
                      ...
                      {order.eventInfo && (
                        <div className="flex items-center gap-4 mt-2 text-sm text-gray-600">
                          <div className="flex items-center gap-1">
                            <Calendar size={14} />
                            <span>{order.eventInfo.eventDate ? new Date(order.eventInfo.eventDate).toLocaleDateString() : 'N/A'}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <Users size={14} />
                            <span>{order.eventInfo?.guestCount || 0} guests</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <MapPin size={14} />
                            <span className="capitalize">{order.eventInfo?.venueType || 'N/A'}</span>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {order.items?.map((item, index) => (
                      <div key={index} className="flex gap-4 p-4 border rounded-lg">
                        ...
                        <div className="flex-1">
                          <h4 className="font-medium text-gray-900">{item.product?.name || item.productName || item.name || 'Product'}</h4>
                          <p className="text-sm text-gray-600">
                            Quantity: {item.quantity || 0} × ₹{(item.unitPrice || item.price || 0).toFixed(2).toLocaleString()}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-gray-900">
                            ₹{(item.totalPrice || (item.quantity || 0) * (item.unitPrice || item.price || 0)).toFixed(2).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-lg font-bold text-gray-900">
                          ₹{(order.pricing?.total || order.total || 0).toFixed(2).toLocaleString()}
                        </span>
                      </div>
                      {order.tracking && (
