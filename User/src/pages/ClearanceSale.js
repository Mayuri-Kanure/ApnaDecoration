...
const ClearanceSale = () => {
  ...
  const fetchClearanceSaleData = async () => {
    try {
      ...
      const inhouseProducts = clearanceData?.inhouseOffer?.applicableProducts || [];
      const vendorProducts = clearanceData?.vendorOffers?.flatMap(offer => offer.applicableProducts || []) || [];
  ...
  const ProductCard = ({ product, offerType = 'inhouse' }) => {
    ...
    const handleAddToCart = async (e) => {
      ...
      try {
        ...
        if (result.success) {
          success(`${product.displayName || product.name} added to cart at ₹${discountPrice.toFixed(2)}`);
        } else {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-red-600 to-blue-600 text-white py-12">
        <div className="container mx-auto px-4">
          <div className="text-center">
            <h1 className="text-4xl font-bold mb-4">🔥 Clearance Sale</h1>
            <p className="text-xl mb-2">Amazing deals on selected products and services!</p>
            {clearanceData?.inhouseOffer?.isActive && (
              <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4 inline-block">
                <p className="text-lg">
                  Up to <span className="font-bold text-yellow-300">
                    {clearanceData.inhouseOffer.discountAmount}% OFF
                  </span> on selected items
                </p>
                <p className="text-sm mt-1">
                  Valid until: {new Date(clearanceData.inhouseOffer.endDate).toLocaleDateString()}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {allProducts.map((product, index) => (
              <ProductCard 
                ...
                product={product}
                offerType={inhouseProducts.includes(product) ? 'inhouse' : 'vendor'}
              />
            ))}
          </div>
        ) : (
