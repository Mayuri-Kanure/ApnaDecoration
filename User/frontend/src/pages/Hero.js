...
const FALLBACK_IMAGE =
  "https://res.cloudinary.com/drrlkntpx/image/upload/v1770901463/apna-decoration/service-categories/1770901461463-4u587m.jpg";
...
const Hero = () => {
  ...
  useEffect(() => {
    // Clearance sale endpoints don't exist on server - disable to prevent 404 errors
    const fetchClearanceData = async () => {
      console.log(
        "ℹ️ Clearance sale features disabled - endpoints not available",
      );
        ...
        try {
          ...
          const heroSlidesFromDB = bannerArray
            .filter((banner) => {
              ...
              console.log(
                `Banner ${banner._id}: published=${banner.published}, isPublished=${isPublished}, bannerType=${banner.bannerType}, isMainBanner=${isMainBanner}`,
              );
              return isPublished && isMainBanner;
            })
            .map((banner) => {
      ...
      <div className=" bg-gray-50 font-sans text-gray-800">
        ...
        <section className="w-full mt-8">
          ...
          <div className="w-full relative ">
            <Link
              to={
                banners[activeSlide]?.resourceType === "product"
                  ? `/product/${banners[activeSlide]?.product}`
                  : banners[activeSlide]?.resourceType === "category"
                    ? `/products?category=${banners[activeSlide]?.category}`
                    : banners[activeSlide]?.bannerUrl || "/products"
              }
            ...
            >
              {banners.length > 0 && banners[activeSlide]?.image ? (
                (() => {
                  ...
                  if (typeof imgSrc === "object" && imgSrc !== null) {
                    imgSrc =
                      ...
                      imgSrc.src ||
                      imgSrc.secure_url ||
                      Object.values(imgSrc)[0];
                  }

                  // If still not a string or empty, use fallback
                  if (typeof imgSrc !== "string" || !imgSrc) {
                    console.warn(
                      "Invalid or missing banner image, using fallback:",
                      imgSrc,
                    );
                    imgSrc = FALLBACK_IMAGE;
                  }

                  console.log("Banner image src being rendered:", {
                    slideId: slide.id,
                    activeSlide,
                  ...
                  return (
                    <div
                      className="relative w-full md:aspect-[16/6] aspect-[2/1]"
                      style={{
                        transformOrigin: "center",
                      }}
                    >
                      {/* Main image (NO CUT - full image visible) */}
                      <img
                        src={imgSrc}
                ...
                <div
                  ...
                  style={{
                    aspectRatio:
                      banners[activeSlide]?.aspectRatio &&
                      typeof banners[activeSlide]?.aspectRatio === "string"
                        ? banners[activeSlide].aspectRatio
              ...
              {/* HERO OVERLAY - Clean Modern Design */}
              <div
                className="absolute inset-0 flex flex-col justify-between"
                style={{ pointerEvents: "none" }}
              >
                <div></div>
                <div className="text-center text-white px-4 pb-4 md:pb-8">
                  <button
                    onClick={(e) => {
                    ...
                    hover:from-pink-700 hover:to-purple-700 text-white px-4 md:px-6 py-2 md:py-3
                    rounded-full font-semibold text-sm md:text-base shadow-lg transition-all duration-300 hover:scale-105"
        ...
        {isClearanceActive && (
          <section className="w-full px-4 lg:px-6 py-12 bg-gradient-to-r from-red-50 to-orange-50 rounded-lg my-8">
            <div className="text-center mb-8">
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-4">
                Huge Discounts on Selected Decorations
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto mb-6">
                Limited time offers on premium decoration items. Save up to 70%
                        ...
                        <div className="aspect-square bg-gray-100 relative overflow-hidden">
                          {product.thumbnail || product.images?.[0] ? (
                            <img
                              ...
                              onLoad={() => {
                                console.log(
                                  "✅ Clearance product image loaded:",
                                  product.name,
                                );
                              }}
                            />
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex items-center text-sm text-green-600 font-semibold mb-2">
                            <span>
                              Save ₹
                              {(
                                product.originalPrice - product.clearancePrice
                              )?.toFixed(2)}
                            </span>
                          </div>
        ...
        {/* Clearance Sale Section */}
        {isClearanceActive && (
          <section className="w-full px-4 lg:px-6 py-8">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between">
                <div className="mb-6 md:mb-0">
                  ...
                  <p className="text-white/80 mb-4 max-w-2xl">
                    Don't miss out on amazing deals on selected products,
                    services, and vendor items. Limited stock available - grab
                    ...
                    {clearanceData?.inhouseOffer?.endDate ? (
                      <>
                        <div className="text-3xl font-bold mb-1">
                          <CountdownTimer
                            expiryDate={clearanceData.inhouseOffer.endDate}
                          />
                        </div>
        ...
        <section className="w-full px-4 lg:px-6 py-12">
          {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {featuredProducts.slice(0, 4).map((product, index) => (
                  <Link
                    ...
                    className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 relative block focus:outline-none focus:ring-0 active:outline-none"
                    aria-label={`View ${product.name} details`}
                  >
                    {product.discount && (
                      <span className="absolute top-4 left-4 bg-indigo-600 text-white text-xs px-2 py-1 rounded-full font-medium z-10">
                        {product.discount}% OFF
                      </span>
                    )}
                    <span className="absolute top-4 right-4 bg-yellow-500 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 z-10 shadow-md">
                      <Sparkles size={12} />
                      Featured
                    </span>
          ...
          {categoriesLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[...Array(6)].map((_, index) => (
                ...
                >
                  <div className="w-20 h-20 bg-gradient-to-br from-indigo-100 to-slate-100 rounded-xl mx-auto mb-3"></div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {categories.slice(0, 6).map((category) => (
                <Link
                  ...
                  className="p-6 transition-all duration-300 cursor-pointer text-center block"
                  aria-label={`Shop ${category.name} products`}
                >
                  <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 overflow-hidden rounded-lg bg-gradient-to-br from-indigo-100 to-slate-100 mx-auto mb-3 flex items-center justify-center">
                    {category.image ? (
                      <img
                        src={
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[...Array(6)].map((_, index) => (
                <div
                  ...
                  className="bg-white p-6 rounded-xl shadow-sm animate-pulse"
                >
                  <div className="w-20 h-20 bg-gradient-to-br from-pink-100 to-indigo-100 rounded-xl mx-auto mb-3"></div>
                  <div className="h-4 bg-gray-200 rounded mx-auto"></div>
                </div>
              ))}
            </div>
              ...
              {serviceCategories.slice(0, 6).map((serviceCategory) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    window.location.href = `/services?type=${serviceCategory.id || serviceCategory._id}`;
                  }}
                ...
                >
                  <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 overflow-hidden rounded-lg bg-gradient-to-br from-pink-100 to-indigo-100 rounded-xl mx-auto mb-3 flex items-center justify-center">
                      ...
                      <img
                        src={(() => {
                          ...
                          if (typeof imageUrl !== "string") {
                            console.warn(
                              "Invalid service category image format:",
                              imageUrl,
                            );
                            return null;
                          }
                          ...
                          console.log(
                            "Service category image src being rendered:",
                            {
                        ...
                        onError={(e) => {
                          console.error(
                            "Service category image failed to load:",
                            {
                              serviceCategoryId: serviceCategory.id,
                    ...
                    <div
                      className="w-full h-full flex items-center justify-center"
                      style={{
                        display: serviceCategory.image ? "none" : "flex",
                      }}
                    >
        ...
        <section className="w-full px-4 lg:px-6 py-16">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              We're dedicated to making your celebrations unforgettable with
              professional service and quality decorations
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Trusted by thousands of happy customers for their special
                moments
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">
                4.9/5 average rating from satisfied clients across all events
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">
                We stand behind our work with a satisfaction promise
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">
                Expert decorators dedicated to making your event perfect
              </p>
            </div>
          </div>
        </section>
