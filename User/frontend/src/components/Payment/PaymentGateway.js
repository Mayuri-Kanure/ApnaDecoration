...
const PaymentGateway = ({
  ...
  const createRazorpayOrder = async () => {
    try {
      ...
      setRazorpayOrder(response); // ✅ FIX: Use response directly
    } catch (error) {
      setError(
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Direct Razorpay implementation since openRazorpayCheckout doesn't exist
      const options = {
    ...
    <Card sx={{ maxWidth: 600, mx: "auto", mt: 2 }}>
      <CardContent>
        ...
        <Dialog open={paymentDialogOpen} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Typography>
              Please complete your payment in the Razorpay window that will
              open. Do not close this window until the payment is complete.
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button
        ...
        <Box mt={3} p={2} bgcolor="#F8FAFC" borderRadius={1}>
          <Typography variant="body2" color="text.secondary" align="center">
            ...
            <br />
            Multiple payment options available: Credit Card, Debit Card, UPI,
            Net Banking
          </Typography>
        </Box>
      </CardContent>
