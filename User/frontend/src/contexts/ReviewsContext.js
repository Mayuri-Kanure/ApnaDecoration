...
export const ReviewsProvider = ({ children }) => {
  ...
  const getProductReviews = useCallback(async (productId) => {
    const response = await robustFetch(`/reviews/product/${productId}`, {
  ...
  const addReview = useCallback(async (productId, reviewData) => {
    const response = await robustFetch(`/reviews`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({
        productId,
  ...
  const updateReview = useCallback(async (reviewId, updateData) => {
    const response = await robustFetch(`/reviews/${reviewId}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
      body: JSON.stringify({
  ...
  const markHelpful = useCallback(async (reviewId) => {
    const response = await robustFetch(`/reviews/${reviewId}/helpful`, {
      method: 'PATCH',
      headers: getAuthHeaders()
    });
