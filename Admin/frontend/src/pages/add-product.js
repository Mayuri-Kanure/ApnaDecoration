...
export default function AddProduct() {
  ...
  useEffect(() => {
    const isEditRoute = location.pathname === '/dashboard/edit-product' && location.state?.isEdit;
  ...
  useEffect(() => {
    if (isEditMode && editingProduct) {
      // Set form values with the editing product data - use correct field names
      setValue('product_name_en', editingProduct.name || '');
      setValue('description_en', editingProduct.description || '');
      
      // Fix category mapping - convert category name to ID if needed
      let categoryId = editingProduct.category_id;
      if (typeof categoryId === 'string' && !categoryId.match(/^[0-9a-fA-F]{24}$/)) {
        // It's a category name, find the corresponding ID
      ...
      setValue('unit_price', editingProduct.price || '');
      setValue('mrp_price', editingProduct.mrp_price || '');
      setValue('discount_price', editingProduct.discount_amount || '');
      setValue('stock_qty', editingProduct.stock || '');
      setValue('size', editingProduct.size || '');
      setValue('tags', editingProduct.tags || []);
      setValue('status', editingProduct.status || 'active');
      setValue('meta_title', editingProduct.meta_title || '');
      setValue('meta_description', editingProduct.meta_description || '');
      ...
      if (editingProduct.additional_images && Array.isArray(editingProduct.additional_images)) {
        editingProduct.additional_images.forEach((img, index) => {
          existingImages.push({
            url: img,
            type: 'additional',
      ...
      if (editingProduct.meta_image) {
        existingImages.push({
          url: editingProduct.meta_image,
          type: 'meta',
          name: 'meta'
        });
      }
  ...
  }, [isEditMode, editingProduct, setValue, categories]);
...
const [aboutItemInput, setAboutItemInput] = useState('');
  ...
  useEffect(() => {
    ...
    if (editData && editId) {
      const lightweightProduct = JSON.parse(editData);
      
      // Get the full product data with images from localStorage
      const fullProductData = localStorage.getItem(editId);
      const fullProduct = fullProductData ? JSON.parse(fullProductData) : lightweightProduct;
      ...
      reset({
        ...
        sku: fullProduct.sku || '',
        unit_price: fullProduct.unit_price || '',
        discount_type: fullProduct.discount_type || 'flat',
        discount_amount: fullProduct.discount_amount || '',
        tax_percent: fullProduct.tax_percent || '',
        tax_calculation: fullProduct.tax_calculation || 'include',
        shipping_cost: fullProduct.shipping_cost || '',
        shipping_multiply_quantity: fullProduct.shipping_multiply_quantity || false,
        stock_qty: fullProduct.stock_qty || '',
        min_order_qty: fullProduct.min_order_qty || '1',
        max_order_qty: fullProduct.max_order_qty || '',
        tags: fullProduct.tags || [],
        status: fullProduct.status || 'active',
        is_featured: fullProduct.is_featured || false,
        video_link: fullProduct.video_link || '',
        meta_title: fullProduct.meta_title || '',
        meta_description: fullProduct.meta_description || '',
        indexing_option: fullProduct.indexing_option || 'index',
        max_snippet: fullProduct.max_snippet || '-1',
        max_video_preview: fullProduct.max_video_preview || '-1',
        max_image_preview: fullProduct.max_image_preview || 'large',
      });
      
      // Load images from full product data if they exist
      if (fullProduct.thumbnail && fullProduct.thumbnail !== 'EXISTS') {
        setProductThumbnail({ preview: fullProduct.thumbnail });
      }
      if (fullProduct.additional_images && Array.isArray(fullProduct.additional_images) && fullProduct.additional_images[0] !== 'EXISTS') {
        setAdditionalImages(fullProduct.additional_images.map(img => ({ preview: img })));
      }
      if (fullProduct.meta_image && fullProduct.meta_image !== 'EXISTS') {
        setMetaImage({ preview: fullProduct.meta_image });
      }
      
      // Load color-wise images if they exist
      if (fullProduct.color_wise_images && typeof fullProduct.color_wise_images === 'object' && fullProduct.color_wise_images !== 'EXISTS') {
        setColorWiseImages(fullProduct.color_wise_images);
      }
  ...
  async function fetchLists() {
    try {
      ...
      alert('Failed to load categories or attributes. Please check if the backend server is running.');
...
function deleteAboutItemPoint(pointToDelete) {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      fd.append('name', data.product_name_en || data.name || '');
      fd.append('description', data.description_en || data.description || '');
      ...
      fd.append('stock_qty', data.stock_qty || '');
      ...
      fd.append('shipping_multiply_quantity', data.shipping_multiply_quantity || false);
      ...
      fd.append('selected_attributes', JSON.stringify(selectedAttributes || []));
      fd.append('generated_variations', JSON.stringify(generatedVariations || []));
      ...
      additionalImages.forEach((image, index) => {
        if (image.file) {
          fd.append(`additional_image_${index}`, image.file);
        }
      });
      ...
      try {
        ...
        const productData = {
          ...
          has_variations: (generatedVariations && generatedVariations.length > 0) || false,
        ...
        if (productThumbnail && productThumbnail.file) {
          ...
          files.thumbnail = new File([blob], 'thumbnail.jpg', { type: 'image/jpeg' });
        }
        ...
        if (additionalImages && Array.isArray(additionalImages)) {
          ...
          for (const img of additionalImages) {
            if (img.file) {
              ...
              const blob = await response.blob();
              files.additional_images.push(new File([blob], `image_${Date.now()}.jpg`, { type: 'image/jpeg' }));
            }
          }
        }
        ...
        if (metaImage && metaImage.file) {
          ...
          const blob = await response.blob();
          files.meta_image = new File([blob], 'meta_image.jpg', { type: 'image/jpeg' });
        }
        ...
        if (isLocationEditMode) {
          ...
          savedProduct = await apiService.updateProductWithFiles(productId, productData, files);
        } else if (editId) {
          // Update existing product using localStorage (legacy)
          savedProduct = await apiService.updateProductWithFiles(editId, productData, files);
        } else {
          // Create new product
          savedProduct = await apiService.createProductWithFiles(productData, files);
        }
        ...
        const productId = savedProduct.product?._id || savedProduct.product?.id || savedProduct._id || savedProduct.id;
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <TextField
                      ...
                      InputProps={{
                        endAdornment: (
                          <IconButton onClick={generateSKU} size="small" sx={{ color: '#2563eb' }}>
                            <AutorenewIcon />
                          </IconButton>
                        ),
                      }}
                    ...
                    <TextField
                      ...
                      sx={{
                        '& .MuiOutlinedInput-root': {
                          ...
                          '&.Mui-focused': { backgroundColor: 'white', borderColor: '#2563eb', boxShadow: '0 0 0 3px rgba(37, 99, 235, 0.1)' },
                        },
                      }}
                    />
                  )}
  ...
  />
              ...
              <Grid item xs={12} md={4}>
                <Typography variant="body2" sx={{ fontWeight: 600, mb: 1.5, color: '#374151' }}>
                  Discount Amount ({discountType === 'percent' ? '%' : '₹'}) <span style={{ color: '#ef4444' }}>*</span>
                  <Tooltip title={discountType === 'percent' ? 'Percentage discount from unit price' : 'Flat discount amount'}>
                    <InfoIcon sx={{ fontSize: 16, ml: 1, color: '#94a3b8' }} />
                  </Tooltip>
                </Typography>
                <Controller
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <FormControlLabel
                      ...
                      label={field.value ? "Multiply with quantity" : "Flat shipping cost"}
                      sx={{ mt: 1 }}
                    />
                  )}
                />
       ...
       {/* Product Variation Setup Section */}
        ...
        >
          <CardContent sx={{ p: 4 }}>
                    ...
                    <Switch
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSelectedAttributes(prev => prev.filter(attr => attr !== 'size'));
                        }
                      }}
                    />
                  }
            ...
            {(enableColors && selectedColors.length > 0) || (selectedAttributes.includes('size') && selectedSizes.length > 0) ? (
              <Box sx={{ mt: 4, mb: 4 }}>
                <Button
                  variant="contained"
                ...
                <TableContainer component={Paper} elevation={0} sx={{ border: '1px solid #e2e8f0' }}>
                  <Table>
                    <TableHead sx={{ backgroundColor: '#f8fafc' }}>
                      <TableRow>
                        <TableCell sx={{ fontWeight: 600, color: '#374151' }}>SL</TableCell>
                        <TableCell sx={{ fontWeight: 600, color: '#374151' }}>Variant</TableCell>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              {variation.color && (
                                <Chip
                                  ...
                                  size="small"
                                  sx={{ backgroundColor: getColorHex(variation.color), color: 'white' }}
                                />
                              )}
                              {variation.size && (
                                <Chip label={variation.size} size="small" variant="outlined" />
                              )}
                            </Box>
                          </TableCell>
                          <TableCell>
              ...
              <Grid item xs={12} md={4}>
                ...
                {productThumbnail ? (
                  ...
                  >
                    ...
                    <input hidden accept="image/*" type="file" onChange={handleProductThumbnailUpload} />
                  </Button>
                )}
              </Grid>
                  ...
                  {selectedColors.length > 0 ? (
                    selectedColors.map((color) => (
                      <Box key={color} sx={{ position: 'relative' }}>
                        <Typography variant="caption" sx={{ color: '#374151', mb: 1, display: 'block' }}>
                          {color}
                        </Typography>
                        {colorWiseImages[color] ? (
                          ...
                          >
                            ...
                            <input hidden accept="image/*" type="file" onChange={(e) => handleColorImageUpload(color, e)} />
                          </Button>
                        )}
                      </Box>
                    ))
                  ) : (
                    <Box sx={{ p: 3, textAlign: 'center', color: '#94a3b8', backgroundColor: '#f8fafc', borderRadius: 2, border: '1px dashed #e2e8f0' }}>
                      <Typography variant="caption">
                        Select colors in Product Variation Setup to upload color-wise images
                      </Typography>
                    </Box>
                  )}
                </Box>
              ...
              <Grid item xs={12} md={4}>
                ...
                <Box sx={{ mb: 2 }}>
                  ...
                  >
                    ...
                    <input hidden accept="image/*" multiple type="file" onChange={handleAdditionalImagesUpload} />
                  </Button>
                </Box>
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <TextField
                      ...
                      fullWidth
                      error={!!fieldState.error}
                      helperText={fieldState.error?.message || "Example: https://www.youtube.com/embed/5R06LRdUCSE"}
                      sx={{
                        '& .MuiOutlinedInput-root': {
                          borderRadius: 2,
                          backgroundColor: '#f8fafc',
              ...
              <Grid item xs={12} md={6}>
                ...
                {metaImage ? (
                  ...
                  >
                    ...
                    <input hidden accept="image/*" type="file" onChange={handleMetaImageUpload} />
                  </Button>
                )}
              </Grid>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Box>
                      <FormControlLabel
                        control={<Radio checked={field.value === 'index'} onChange={() => field.onChange('index')} />}
                        label="Index"
                        sx={{ mb: 1 }}
                      />
                      <FormControlLabel
                        control={<Radio checked={field.value === 'noindex'} onChange={() => field.onChange('noindex')} />}
                        label="No Index"
                        sx={{ mb: 2 }}
                      />
