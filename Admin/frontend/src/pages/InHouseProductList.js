...
function InHouseProductList() {
  ...
  const brands = ['All Brands', 'AudioTech', 'EcoWear', 'FitTech', 'HydroMax', 'PhotoPro', 'YogaLife', 'DigitalPro', 'ChargeTech'];
  const categories = ['Select category', 'Electronics', 'Fashion', 'Sports', 'Digital'];
  const subCategories = {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesBrand = brandFilter === 'all' || brandFilter === 'All Brands' || product.brand === brandFilter;
  ...
  const generateBarcode = () => {
    ...
    ctx.fillStyle = '#000';
    const barcodeData = selectedProductForBarcode.id.toString().padStart(10, '0');
  ...
  const handleExport = () => {
    const csvContent = [
      ['SL', 'Product Name', 'Product Type', 'Unit Price', 'Featured', 'Active Status'],
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      {loading && (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(255,255,255,0.8)', zIndex: 9999 }}>
          <CircularProgress />
        </Box>
      )}
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={6} md={2.4}>
              <FormControl fullWidth size="small">
                ...
                >
                  {brands.map((brand) => (
                    <MenuItem key={brand} value={brand === 'All Brands' ? 'all' : brand}>
                      {brand}
                    </MenuItem>
                  ))}
                </Select>
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  ...
                  {categories.map((category) => (
                    <MenuItem key={category} value={category === 'Select category' ? 'all' : category}>
                      {category}
                    </MenuItem>
                  ))}
                </Select>
            ...
            <Grid item xs={12} sm={6} md={2.4}>
              <FormControl fullWidth size="small" disabled={categoryFilter === 'all' || !subCategories[categoryFilter]}>
                ...
                >
                  ...
                  {categoryFilter !== 'all' && subCategories[categoryFilter]?.map((subCat) => (
            ...
            <Grid item xs={12} sm={6} md={2.4}>
              <FormControl fullWidth size="small" disabled={subCategoryFilter === 'all' || !subSubCategories[subCategoryFilter]}>
                ...
                >
                  ...
                  {subCategoryFilter !== 'all' && subSubCategories[subCategoryFilter]?.map((subSubCat) => (
          ...
          <TableContainer component={Paper} elevation={0}>
            <Table>
              <TableHead sx={{ backgroundColor: '#fafafa' }}>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>SL</TableCell>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>Product Name</TableCell>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>Product Type</TableCell>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>Unit Price</TableCell>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>Show as Featured</TableCell>
                  <TableCell sx={{ fontWeight: 600, color: '#333', borderBottom: '1px solid #e0e0e0' }}>Active Status</TableCell>
              ...
              <TableBody>
                {paginatedProducts.map((product, index) => (
                  ...
                  >
                    <TableCell sx={{ borderBottom: '1px solid #e0e0e0' }}>{page * rowsPerPage + index + 1}</TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #e0e0e0' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        <Avatar 
                          sx={{ width: 40, height: 40, backgroundColor: '#f5f5f5' }}
                          src={getProductImage(product.name, product.productType)}
                        >
                          {!getProductImage(product.name, product.productType) && <ProductIcon sx={{ fontSize: 20, color: '#999' }} />}
                        </Avatar>
                    ...
                    <TableCell sx={{ borderBottom: '1px solid #e0e0e0' }}>
                      <Chip 
                        ...
                        size="small"
                        color={product.productType === 'Digital' ? 'primary' : 'default'}
                        variant="outlined"
                      />
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #e0e0e0' }}>
          ...
          {selectedProduct && (
            <Grid container spacing={2}>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Product ID:</Typography>
                <Typography variant="body1" sx={{ mb: 2 }}>{selectedProduct.id}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Typography variant="body1" sx={{ mb: 2 }}>{selectedProduct.brand}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Product Type:</Typography>
                <Typography variant="body1" sx={{ mb: 2 }}>{selectedProduct.productType}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Unit Price:</Typography>
                <Typography variant="body1" sx={{ mb: 2, color: '#1976d2', fontWeight: 600 }}>
                  ₹{selectedProduct.unitPrice.toFixed(2)}
                </Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Stock:</Typography>
                <Typography variant="body1" sx={{ mb: 2 }}>{selectedProduct.stock} units</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
              ...
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Sub Category:</Typography>
                <Typography variant="body1" sx={{ mb: 2 }}>{selectedProduct.subCategory}</Typography>
              </Grid>
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Featured:</Typography>
                <Chip 
              ...
              <Grid item xs={12} md={6}>
                <Typography variant="body2" sx={{ color: '#666' }}>Active Status:</Typography>
                <Chip 
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="lg" fullWidth>
        ...
        <DialogContent>
          {selectedProduct && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  ...
                  <Select
                    ...
                    label="Category"
                  >
                    {categories.filter(c => c !== 'Select category').map((category) => (
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  ...
                  <Select
                    ...
                    onChange={(e) => setSelectedProduct({...selectedProduct, subSubCategory: e.target.value})}
                    label="Sub Sub Category"
                  >
                    {subSubCategories[selectedProduct.subCategory]?.map((subSubCat) => (
                      <MenuItem key={subSubCat} value={subSubCat}>{subSubCat}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
              </Grid>
              <Grid item xs={12} md={6}>
                <Box sx={{ mt: 2 }}>
                  <Typography variant="body2" sx={{ mb: 1 }}>Featured Product</Typography>
                  <Switch
              ...
              <Grid item xs={12} md={6}>
                <Box sx={{ mt: 2 }}>
                  <Typography variant="body2" sx={{ mb: 1 }}>Active Status</Typography>
                  <Switch
        ...
        <DialogActions>
          ...
          <Button onClick={saveEditedProduct} variant="contained" color="primary" disabled={loading}>
          ...
          {selectedProductForBarcode && (
            <Box sx={{ textAlign: 'center', py: 2 }}>
              <Typography variant="body1" sx={{ mb: 2 }}>
                Generate barcode for: <strong>{selectedProductForBarcode.name}</strong>
              </Typography>
