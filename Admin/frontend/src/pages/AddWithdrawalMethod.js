...
function AddWithdrawalMethod() {
  ...
  const handleSubmit = async () => {
    ...
    if (invalidFields.length > 0) {
      setSnackbar({
        open: true,
        message: "All field names are required",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message:
          error.response?.data?.message || "Error creating withdraw method",
        severity: "error",
      });
    }
  };
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth variant="outlined">
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleFieldChange(field.id, "inputType", e.target.value)
                    }
