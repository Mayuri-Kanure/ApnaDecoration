...
export default function AddProduct() {
  ...
  // New states for product specifications
  ...
  const compressBase64Image = async (base64String, quality = 0.7) => {
    return new Promise((resolve) => {
      const img = new Image();
      img.onload = () => {
        const canvas = document.createElement('canvas');
        ...
        // Reduce dimensions for thumbnails
        const maxWidth = 600;
        const maxHeight = 600;
        let { width, height } = img;
  ...
  useEffect(() => {
    ...
    if (editData && editId) {
      ...
      console.log('Loading lightweight product for edit:', lightweightProduct);
  ...
  async function fetchLists() {
    try {
      ...
      console.log('Attributes response:', attributesData.data);
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      console.log('Saving product data via API...');
      ...
      try {
        ...
        console.log('Product data prepared for API:', productData);
        console.log('Files prepared for upload:', Object.keys(files));
        ...
        if (editId) {
          ...
          console.log('Product updated via API:', savedProduct);
        } else {
          // Create new product
          savedProduct = await apiService.createProductWithFiles(productData, files);
          console.log('Product created via API:', savedProduct);
        }
        
        // Clear edit ID if editing
        if (editId) {
          localStorage.removeItem('editProductId');
          localStorage.removeItem('editProductData');
        }
        ...
        navigate(`/dashboard/products/${savedProduct._id || savedProduct.id}`, { replace: true });
        ...
        try {
          const editId = localStorage.getItem('editProductId');
          const productId = editId || `product_${Date.now()}`;
          
          // Simple localStorage save without compression for fallback
          ...
          localStorage.setItem(productId, JSON.stringify(fallbackProductData));
          
          const action = editId ? 'updated' : 'saved';
          alert(`Product ${action} successfully! (Saved locally - API failed)`);
          
          navigate(`/dashboard/products/${productId}`, { replace: true });
        } catch (fallbackError) {
          console.error('Fallback save also failed:', fallbackError);
          alert(`Failed to save product: ${apiError.message || 'Unknown error'}`);
        }
      }
        ...
        <Paper 
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={4}>
                <Typography variant="body2" sx={{ fontWeight: 600, mb: 1.5, color: '#374151' }}>
                  Current Stock Qty <span style={{ color: '#ef4444' }}>*</span>
                </Typography>
                <Controller
                  name="stock_qty"
                  control={control}
                  rules={{ required: 'Stock qty is required', min: 0 }}
                  render={({ field, fieldState }) => (
                    <TextField
                      {...field}
                      type="number"
