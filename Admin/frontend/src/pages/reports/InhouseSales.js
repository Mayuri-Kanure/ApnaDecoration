...
const InhouseSales = () => {
  ...
  const [categories, setCategories] = useState([{ value: 'all', label: 'All' }]); // Initialize with 'all' option
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/inhouse-sales/categories`, {
  ...
  const fetchSalesData = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/inhouse-sales?page=${page}&limit=10&category=${selectedCategory}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Use fallback data silently (no error message to user)
  ...
  }, [page, selectedCategory, fetchSalesData]);
            ...
            <FormControl size="medium" sx={{ minWidth: 200 }}>
              ...
              >
                {categories.map((category, index) => (
                  <MenuItem key={`${category.value}-${index}`} value={category.value}>
                    {category.label}
                  </MenuItem>
                ))}
              </Select>
