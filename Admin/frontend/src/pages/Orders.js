...
function Orders({ status: statusProp, showCancelled = false, title }) {
  ...
  const pageTitle = title || (currentStatus === 'all' ? 'All Orders' : `${currentStatus.charAt(0).toUpperCase() + currentStatus.slice(1)} Orders`);
  ...
  const fetchOrders = async () => {
    try {
      ...
      console.log('🔍 ADMIN FRONTEND - Orders count:', response.data.orders?.length);
  ...
  const handleExportData = () => {
    ...
    const csvContent = [
      ['SL', 'Order ID', 'Order Date', 'Customer Name', 'Mobile', 'Store', 'Total Amount', 'Payment Status', 'Order Status'],
    ...
    <Box>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box display="flex" alignItems="center" gap={2}>
          ...
          <Badge badgeContent={totalOrders} color="primary" sx={{ '& .MuiBadge-badge': { fontSize: '0.875rem', fontWeight: 500 } }}>
            <Box />
          </Badge>
        </Box>
      </Box>
      ...
      <Paper sx={{ p: 3, mb: 3, bgcolor: 'white' }}>
        <Typography variant="h6" mb={2} fontWeight="500">Filter Orders</Typography>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Box display="flex" gap={1}>
              <Button 
                ...
                sx={{ borderColor: '#d1d5db', color: '#6b7280', '&:hover': { borderColor: '#9ca3af' } }}
              >
                Reset
              </Button>
              <Button 
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" gap={2} flexDirection={{ xs: 'column', sm: 'row' }}>
          ...
          <Button
            ...
            sx={{ bgcolor: '#10b981', '&:hover': { bgcolor: '#059669' }, width: { xs: '100%', sm: 'auto' } }}
          >
            Export
          </Button>
        </Box>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <TableContainer component={Paper} sx={{ maxHeight: 600, minWidth: 800 }}>
          ...
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>SL</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Order ID</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Order Date</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Customer Info</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Store</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Total Amount</TableCell>
              <TableCell sx={{ fontWeight: 600, bgcolor: '#f8fafc' }}>Order Status</TableCell>
          ...
          <TableBody>
            {orders.map((order, index) => (
              <TableRow key={order._id} hover sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                ...
                <TableCell sx={{ fontWeight: 500 }}>{order.orderNumber || `#${order._id?.slice(-8)}`}</TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Chip 
                    ...
                    size="small" 
                    variant="outlined"
                    sx={{ borderColor: order.store === 'vendor' ? '#f59e0b' : '#6366f1', color: order.store === 'vendor' ? '#f59e0b' : '#6366f1' }}
                  />
                </TableCell>
                <TableCell>
