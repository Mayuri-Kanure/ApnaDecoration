...
const ClearanceSale = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/clearance-sale/products`,
        {
  ...
  const fetchVendors = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${API_BASE_URL}/clearance-sale/vendors`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchClearanceData = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: "Error loading clearance data",
        severity: "error",
      });
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (clearanceData?.inhouseOffer) {
      setFormData({
        ...
        startDate: clearanceData.inhouseOffer.startDate
          ? new Date(clearanceData.inhouseOffer.startDate)
              .toISOString()
              .split("T")[0]
          : "",
        endDate: clearanceData.inhouseOffer.endDate
          ? new Date(clearanceData.inhouseOffer.endDate)
              .toISOString()
              .split("T")[0]
          : "",
    ...
    if (clearanceData?.prioritySettings) {
      ...
      setDefaultSortBy(
        clearanceData.prioritySettings.defaultSortBy || "createdAt",
      );
  ...
  const handleUpdateInhouseOffer = async () => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/clearance-sale/inhouse-offer`,
        inhouseOfferData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      setSnackbar({
        open: true,
        message: "Error updating inhouse offer",
        severity: "error",
      });
    }
  };
  ...
  const handleSaveVendorOffer = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/clearance-sale/vendor-offers`,
        vendorOfferData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleDeleteVendor = async (vendorId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `${API_BASE_URL}/clearance-sale/vendor-offers/${vendorId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      setVendorOffers(
        vendorOffers.filter((offer) => offer.vendorId !== vendorId),
      );
      setSnackbar({
        open: true,
  ...
  const handleSavePrioritySetup = async () => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/clearance-sale/priority-settings`,
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Typography
                    ...
                    sx={{
                      fontWeight: 500,
                      color: clearanceData?.inhouseOffer?.isActive
                        ? "success.main"
                        : "text.secondary",
                    }}
                  >
                        ...
                        <TableRow key={`product-${product._id}-${index}`} hover>
                          ...
                          <TableCell>
                            ...
                            >
                              ...
                              >
                                {(product.displayName || product.name || "P")
                                  ?.charAt(0)
                                  ?.toUpperCase() || "P"}
                              </Avatar>
                              <Typography
        ...
        <>
          ...
          <Card sx={{ mb: 4 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              >
                <Typography variant="h6">Vendor Offers Management</Typography>
                <Button
                  variant="contained"
                  startIcon={<AddIcon />}
              ...
              <Typography variant="body2" sx={{ mb: 2, color: "#666" }}>
                Add vendor-specific clearance offers to include their products
                in the sale
              </Typography>
            </CardContent>
          </Card>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              {vendorOffers.length === 0 ? (
                <Box sx={{ textAlign: "center", py: 8 }}>
                  ...
                  <Typography variant="body2" sx={{ color: "#999", mb: 3 }}>
                    Add vendor offers to include their products in clearance
                    sale
                  </Typography>
                  <Button
                    variant="outlined"
                          ...
                          <TableCell>
                            <Box sx={{ display: "flex", gap: 1 }}>
                              ...
                              <IconButton
                                ...
                                color="error"
                                onClick={() =>
                                  handleDeleteVendor(offer.vendorId)
                                }
                              >
                                <DeleteIcon fontSize="small" />
                              </IconButton>
          ...
          >
            ...
            <DialogActions>
              ...
              <Button onClick={handleSaveVendorOffer} variant="contained">
                Save Offer
              </Button>
            </DialogActions>
          </Dialog>
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <Button
                      variant="outlined"
                      size="small"
                      onClick={handleResetPriorityOrder}
                    >
                      Reset Order
                    </Button>
                  </Box>
                      ...
                      <Card variant="outlined" sx={{ p: 2 }}>
                        ...
                        >
                          ...
                          <Switch
                            ...
                            onChange={(e) =>
                              handleDefaultSortingChange(e.target.checked)
                            }
                            color="primary"
                            size="small"
                          />
                      ...
                      <Card variant="outlined" sx={{ p: 2 }}>
                        ...
                        >
                          ...
                          <Switch
                            ...
                            onChange={(e) =>
                              handleCustomSortingChange(e.target.checked)
                            }
                            color="primary"
                            size="small"
                          />
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="body2" sx={{ color: "#666" }}>
                        Configure how clearance sale items are displayed and
                        sorted
                      </Typography>
                    </Box>
