...
class ApiService {
  ...
  async updateProductWithFiles(id, productData, files) {
    try {
      ...
      if (files.additional_images && files.additional_images.length > 0) {
        ...
        console.log('🔧 Added additional images to files array:', files.additional_images.length, 'files');
      }
  ...
  async getFlashDeals(params = {}) {
    try {
      ...
      const response = await this.request(`/flash-deals${queryString ? '?' + queryString : ''}`);
  ...
  async addProductToFlashDeal(flashDealId, productData) {
    try {
      const response = await this.request(`/flash-deals/${flashDealId}/products`, {
        method: 'POST',
        body: JSON.stringify(productData)
      });
      return response.data || response;
  ...
  async removeProductFromFlashDeal(flashDealId, productId) {
    try {
      const response = await this.request(`/flash-deals/${flashDealId}/products/${productId}`, {
        method: 'DELETE'
      });
      return response.data || response;
    } catch (error) {
