...
const GeneralSettings = () => {
  ...
  const [digitalProductDialogOpen, setDigitalProductDialogOpen] = useState(false);
  const [maintenanceConfirmDialogOpen, setMaintenanceConfirmDialogOpen] = useState(false);
  const [pendingMaintenanceMode, setPendingMaintenanceMode] = useState(false);
  const [pendingMaintenanceModeOriginal, setPendingMaintenanceModeOriginal] = useState(false);
  ...
  const handleMaintenanceModeConfirm = async () => {
    try {
      ...
      setAlert({
        open: true,
        message: 'Failed to update maintenance mode: ' + (error.response?.data?.message || error.message),
        severity: 'error'
      });
    } finally {
      setLoading(false);
  ...
  const handleMaintenanceModeCancel = () => {
    ...
    setForm(prev => ({ ...prev, maintenanceMode: pendingMaintenanceModeOriginal }));
    setMaintenanceConfirmDialogOpen(false);
    setPendingMaintenanceMode(false);
    setPendingMaintenanceModeOriginal(false);
  };
  ...
  return (
    <>
          ...
          <CardContent>
            ...
            <FormControlLabel
              ...
              label={
                <Box 
                  ...
                  sx={{ cursor: 'pointer' }}
                >
                  Maintenance Mode {form.maintenanceMode ? '(ENABLED)' : '(DISABLED)'}
                </Box>
              }
            />
            ...
            {form.maintenanceMode && (
              <Typography variant="body2" color="error.main" sx={{ mt: 1, fontWeight: 'bold' }}>
                ⚠️ To disable maintenance mode, please go to the Order tab and use the toggle there.
              </Typography>
            )}
          </CardContent>
        </Card>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>
            When enabled, the website becomes inaccessible to users. Only administrators can access the system.
          </Typography>
          ...
          <Typography variant="body2" color="warning.main" sx={{ mt: 2, fontWeight: 'bold' }}>
            Warning: Enabling maintenance mode will immediately make your website unavailable to all users.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>
            Are you sure you want to {pendingMaintenanceMode ? 'enable' : 'disable'} maintenance mode?
          </Typography>
          {pendingMaintenanceMode ? (
            <>
              <Typography variant="body2" color="warning.main" sx={{ mb: 2, fontWeight: 'bold' }}>
                ⚠️ WARNING: This will make your website inaccessible to all users!
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Only administrators will be able to access the system while maintenance mode is enabled.
              </Typography>
            </>
          ) : (
            <Typography variant="body2" color="text.secondary">
              Disabling maintenance mode will make your website accessible to all users again.
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
          ...
          <Button 
            ...
            disabled={loading}
          >
            {loading ? 'Processing...' : `Yes, ${pendingMaintenanceMode ? 'Enable' : 'Disable'} Maintenance Mode`}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogContent sx={{ pt: 3, pb: 2 }}>
          <Typography variant="body1" sx={{ mb: 3, lineHeight: 1.6 }}>
            By enabling this feature you can sell digital products like e-books, PDFs, software, music, videos, and other downloadable items on your platform.
          </Typography>
