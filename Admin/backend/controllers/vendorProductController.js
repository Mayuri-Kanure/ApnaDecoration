...
exports.createVendorProduct = async (req, res) => {
  try {
    ...
    if (req.body.productData) {
      ...
      console.log(
        "Using direct req.body as product data:",
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Stock is required. Please enter the stock quantity.",
      });
    }
    ...
    // Add upper safety limit
    if (stockValue > 100000) {
      return res.status(400).json({
        success: false,
        message: "Stock value too large. Maximum allowed is 100000.",
      });
    }

    // Update productData with validated stock
...
exports.updateVendorProduct = async (req, res) => {
  try {
    ...
    const updatedProduct = await VendorProduct.findByIdAndUpdate(id, updates, {
      new: true,
    });
...
exports.getVendorProductsByStatus = async (req, res) => {
  try {
    ...
    const products = await VendorProduct.find({ status: status })
      ...
      .select(
        "name sku price description category status thumbnail images vendorId createdAt updatedAt approvedAt",
      )
      .sort(options.sort)
      .limit(options.limit);
...
exports.toggleProductStatus = async (req, res) => {
  try {
    ...
    product.active_status =
      product.active_status === "active" ? "inactive" : "active";
    await product.save();

    const updatedProduct = await VendorProduct.findById(product._id).populate(
      "vendor",
...
exports.exportProducts = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=vendor-products.csv",
    );
    res.send(csvContent);
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
