...
const deliveryBoySchema = new mongoose.Schema(
  {
    ...
    vehicleType: {
      ...
      enum: {
        ...
        message:
          "Vehicle type must be one of: motorcycle, scooter, bicycle, car, van",
      },
    },
    vehicleNumber: {
      type: String,
    ...
    bankDetails: {
      ...
      ifscCode: {
        ...
        match: [
          /^[A-Z]{4}[0A-Z0-9]{6,7}$/,
          "Please enter a valid IFSC code (e.g., SBIN0001234 or SBIN0123456)",
        ],
      },
      bankName: {
        type: String,
    ...
    deliveryBoyId: {
      type: String,
      // Temporarily removed unique constraint to fix duplicate key errors
      // unique: true,
      // sparse: true
    },
    employeeId: {
...
deliveryBoySchema.pre("save", async function (next) {
  if (!this.deliveryBoyId) {
    try {
      ...
      this.deliveryBoyId = `DB${timestamp}${random}`;

      // Also generate employee ID if needed
      if (!this.employeeId) {
      ...
      this.deliveryBoyId = `DB${Date.now()}`;
...
// deliveryBoySchema.index({ deliveryBoyId: 1 }); // Removed to fix duplicate key error
